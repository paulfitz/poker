...
/* Our includes */
#include "KLTbase.h"
#include "KLTconvolve.h"
#include "KLTerror.h"
#include "KLTklt.h"
#include "KLTpyramid.h"
...
static void** _createArray2D(int ncols, int nrows, int nbytes)
{
  ...
  char *tmp = new char[nrows * sizeof(void *) + ncols * nrows * nbytes];
  tt = (char **) tmp;
...
KLT_TrackingContext KLTCreateTrackingContext()
{
  ...
  /* Allocate memory */
  tc = (KLT_TrackingContext)  new KLT_TrackingContextRec;
	 // malloc(sizeof(KLT_TrackingContextRec));
  ...
  int nFrames)
{
  ...
  char *tmp = new char [nbytes];
  fh = (KLT_FeatureHistory)  tmp; //malloc(nbytes);
  ...
  int nFeatures)
{
  ...
  /* Allocate memory for feature history */
  char *tmp = new char[nbytes];
  ft = (KLT_FeatureTable)  tmp; // malloc(nbytes);
  ...
  tmp = new char[nFrames * nFeatures * sizeof(KLT_FeatureRec)];
  first = (KLT_Feature) tmp; //malloc();
