...
#define SWAP3(l,i,j)               {register int *pi, *pj, tmp;                 pi=l+3*(i); pj=l+3*(j);                                                   tmp=*pi;         *pi++=*pj;       *pj++=tmp;                        tmp=*pi;         *pi++=*pj;       *pj++=tmp;                        tmp=*pi;         *pi=*pj;         *pj=tmp;    }
  ...
  KLT_BOOL overwriteAllFeatures)
{
  ...
  featuremap = (uchar *) new uchar[ncols*nrows]; //malloc(ncols * nrows * sizeof(uchar));
  ...
  selectionMode mode)
{
  ...
  pointlist = (int *) new int[ncols*nrows*3]; //malloc(ncols * nrows * 3 * sizeof(int));
