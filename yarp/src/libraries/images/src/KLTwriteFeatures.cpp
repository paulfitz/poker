...
#include "KLTbase.h"
#include "KLTerror.h"
#include "KLTpnmio.h"		/* ppmWriteFileRGB() */
#include "KLTklt.h"
  ...
  char *filename)
{
  ...
  redimg = (uchar *)  new uchar[nbytes]; //malloc(nbytes);
  grnimg = (uchar *)  new uchar[nbytes]; //malloc(nbytes);
  bluimg = (uchar *)  new uchar[nbytes]; //malloc(nbytes);
