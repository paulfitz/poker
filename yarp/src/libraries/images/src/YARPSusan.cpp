...
/**********************************************************************\
  ...
  susan_setup_brightness_lut() the LUT is setup. This mallocs enough space
...
/* }}} */
/* {{{ susan_setup_brightness_lut(bp,thresh,form) */
...
/* }}} */
/* {{{ susan_enlarge(in,tmp_image,x_size,y_size,border) */
...
uchar * susan_alloc_smooth_tmp_image (int three_by_three, float dt, int x_size, int y_size)
{
	...
	if ( (2*mask_size+1>x_size) || (2*mask_size+1>y_size) )
	{
		...
		printf("YARP: return without processing\n");
		return NULL;
	}

	uchar *tmp_image = new uchar[(x_size+mask_size*2) * (y_size+mask_size*2)];
...
uchar * susan_alloc_smooth_mask (int three_by_three, float dt, int x_size, int y_size)
{
	...
	{ 
		...
		int n_max = (mask_size*2) + 1;
		uchar *dp = new uchar[n_max*n_max];
		assert (dp != NULL);
...
/* {{{ susan_edge_draw(in,corner_list,drawing_mode) */
...
/* {{{ susan_corner_draw(in,corner_list,drawing_mode) */
