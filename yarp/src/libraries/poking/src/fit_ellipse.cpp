...
public:
  void ROTATE(double a[7][7], int i, int j, int k, int l, double tau, double s) 
    ...
    /**    SINGULAR .                                                  **/
    /********************************************************************/
    int inverse(double TB[3][3], double InvB[3][3], int N) {  
      int k,i,j,p,q;
      static double mult;
    ...
    int inverse7(double TB[7][7], double InvB[7][7], int N) {  
      int k,i,j,p,q;
      static double mult;
      static double D,temp;
      static double maxpivot;
		 ...
		 double& la, double& lb)  
{
  ...
  fitter.Apply(x_in,y_in,num_points,xc,yc,xa,ya,la,lb);
  double eps = 1.0e-5;
  if (fabs(xa)>fabs(ya))
    {
      if (xa<0)
  ...
  if (fabs(xc-0)<eps) xc = 0;
  if (fabs(yc-0)<eps) yc = 0;
  if (fabs(xa-0)<eps) xa = 0;
  if (fabs(ya-0)<eps) ya = 0;
  if (fabs(la-0)<eps) la = 0;
  if (fabs(lb-0)<eps) lb = 0;
}
